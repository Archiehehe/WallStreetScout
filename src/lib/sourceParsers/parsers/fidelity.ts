...
export const fidelityParser: SourceParser = {
  key: 'fidelity',
  displayName: 'Fidelity',

  async discoverArticleUrls(_source: Source): Promise<DiscoveredUrl[]> {
    const urls = await fetchAndParseSitemap('https://www.fidelity.com/sitemap.xml', 2)
    return urls
      .filter((u) => /^\/thinking\//i.test(new URL(u).pathname))

...
export const schwabParser: SourceParser = {
  key: 'schwab',
  displayName: 'Charles Schwab',

  async discoverArticleUrls(_source: Source): Promise<DiscoveredUrl[]> {
    const urls = await fetchAndParseSitemap('https://www.schwab.com/sitemap.xml', 2)
    return urls
      .filter((u) => /^\/learn\//i.test(new URL(u).pathname))
      .filter((u) => !/\/(about|careers|contact)\//i.test(new URL(u).pathname))

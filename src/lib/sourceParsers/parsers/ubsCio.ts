...
export const ubsCioParser: SourceParser = {
  key: 'ubsCio',
  displayName: 'UBS CIO',

  async discoverArticleUrls(_source: Source): Promise<DiscoveredUrl[]> {
    const urls = await fetchAndParseSitemap('https://www.ubs.com/sitemap.xml', 2)
    return urls
      .filter((u) => /^\/global\/en\/(investment-bank\/in-focus|wealth-management\/insights)\//i.test(new URL(u).pathname))
